...
mathedFiles = FindFilesViaExt(appDirectory, "asc");
LogNormal($"Finded {mathedFiles.Count} .asc files");
...
{
    LogInfo("No matched asc files in this folder. Exit.");
...
foreach (string file in mathedFiles)
{
    try
    {
        ...
        string savePath = Path.ChangeExtension(file, ".trc");
